...
import { e as error, g as getDefaultWinProperties, f as fetch } from "./main-DxYEjsed.js";
const temp = require$$1.join(os.tmpdir(), "msmc");
var firefox = false;
var start;
      ...
      for (var i2 = 0; i2 < compatibleW.length; i2++) {
        for (var i = 0; i < pathsW.length; i++) {
          ...
          try {
            ...
            var out = execSync('"C:\\Windows\\System32\\reg.exe" query "' + locW + compatibleW[i2] + '"').toString();
      ...
      for (var i2 = 0; i2 < compatibleL.length; i2++) {
        for (var i3 = 0; i3 < edd.length; i3++) {
          for (var i = 0; i < pathsL.length; i++) {
            const s = require$$1.join(pathsL[i], compatibleL[i2] + edd[i3]);
            if (fs.existsSync(s)) {
              start = s;
...
function browserLoop(auth, port, browser) {
  return new Promise((resolve, error2) => {
    const call = () => {
      try {
        clearInterval(f3);
        process.removeListener("exit", call);
        if (os.type() == "Windows_NT") {
          execSync("taskkill /pid " + browser.pid);
        } else {
          browser.kill();
        }
      } catch {
...
const raw = (auth, Windowproperties = getDefaultWinProperties()) => {
  ...
  return new Promise((resolve, error2) => {
    ...
    const ouput = (out) => {
      ...
      if (firstrun && cout.startsWith("devtools listening on ws://")) {
        ...
        browserLoop(auth, port, browser).then(resolve).catch(error2);
      }
    };
