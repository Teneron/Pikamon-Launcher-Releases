...
function downloadFile(url, dest, label = "Baixando Arquivos...") {
  return new Promise((resolve, reject) => {
    ...
    const file2 = fs$j.createWriteStream(dest);
...
function getJava21Path() {
  const javaRoot = "C:\\Program Files\\Java";
  const defaultPath = path$m.join(javaRoot, "jdk-21", "bin", "java.exe");
  if (fs$j.existsSync(defaultPath)) {
    return defaultPath;
  }
  if (fs$j.existsSync(javaRoot)) {
    try {
      const entries = fs$j.readdirSync(javaRoot);
      const jdk21Folder = entries.find((entry) => entry.startsWith("jdk-21") && fs$j.existsSync(path$m.join(javaRoot, entry, "bin", "java.exe")));
      if (jdk21Folder) {
        return path$m.join(javaRoot, jdk21Folder, "bin", "java.exe");
      }
    } catch (e) {
...
async function ensureJava21() {
  ...
  const installerPath = path$m.join(require$$1$3.app.getPath("temp"), "jdk-21_windows-x64_bin.exe");
  try {
    ...
    if (!fs$j.existsSync(installerPath) || fs$j.statSync(installerPath).size < 1e6) {
    ...
    await new Promise((resolve, reject) => {
      const process2 = node_child_process.spawn(installerPath, [], {
        stdio: "ignore",
        windowsHide: false,
        shell: true
      });
    ...
    while (attempts < maxAttempts) {
      ...
      attempts++;
      const checkPath3 = getJava21Path();
      if (checkPath3) {
        foundPath = checkPath3;
        break;
      }
      if (attempts % 5 === 0) {
    ...
    if (fs$j.existsSync(installerPath)) {
      try {
        fs$j.unlinkSync(installerPath);
      } catch (e) {
      }
    }
  }
}
async function ensureNeoForge(root2, neoVersion) {
  const versionFolder = path$m.join(root2, "versions", `neoforge-${neoVersion}`);
  if (fs$j.existsSync(versionFolder)) {
  ...
  const installerPath = path$m.join(require$$1$3.app.getPath("temp"), `neoforge-${neoVersion}-installer.jar`);
  try {
    ...
    const profilesPath = path$m.join(root2, "launcher_profiles.json");
    if (!fs$j.existsSync(profilesPath)) {
      fs$j.writeFileSync(profilesPath, JSON.stringify({}, null, 2));
    }
    await new Promise((resolve, reject) => {
      const java = "java";
      const process2 = node_child_process.spawn(java, ["-jar", installerPath, "--installClient", root2], {
        stdio: "ignore",
        // Ignore stdio to prevent console window if possible
        windowsHide: true
        // Explicitly hide window on Windows
      });
    ...
    if (fs$j.existsSync(installerPath)) fs$j.unlinkSync(installerPath);
  }
}
async function syncMods(root2) {
  ...
  const modsDir = path$m.join(root2, "mods");
  const modsInfoPath = path$m.join(root2, "mods_info.json");
  const urlPath = modsUrl.split("?")[0];
  const isRarUrl = urlPath.endsWith(".rar");
  const zipPath = path$m.join(require$$1$3.app.getPath("temp"), isRarUrl ? "mods.rar" : "mods.zip");
  try {
    ...
    try {
      ...
      if (fs$j.existsSync(modsInfoPath)) {
        try {
          const localInfo = JSON.parse(fs$j.readFileSync(modsInfoPath, "utf-8"));
      ...
      if (fs$j.existsSync(modsDir) && fs$j.readdirSync(modsDir).length > 0) {
        shouldDownload = false;
    ...
    try {
      ...
      const stat2 = fs$j.statSync(zipPath);
      if (stat2.size < 1e3) {
        throw new Error(`Arquivo muito pequeno (${stat2.size} bytes). Download incompleto ou link quebrado.`);
      }
    } catch (dErr) {
    ...
    try {
      const fd = fs$j.openSync(zipPath, "r");
      const buffer = Buffer.alloc(4);
      fs$j.readSync(fd, buffer, 0, 4, 0);
      fs$j.closeSync(fd);
    ...
    try {
      ...
      zip.extractAllTo(root2, true);
      fs$j.writeFileSync(modsInfoPath, JSON.stringify({ etag: remoteEtag, size: remoteSize }, null, 2));
    ...
    if (fs$j.existsSync(zipPath)) fs$j.unlinkSync(zipPath);
  }
}
...
let tray = null;
function createTray() {
  const iconPath = path$m.join(process.env.VITE_PUBLIC, "icon.png");
  const icon = require$$1$3.nativeImage.createFromPath(iconPath).resize({ width: 16, height: 16 });
...
require$$1$3.ipcMain.handle("auth:login", async (_event, args) => {
  try {
    const username = args.username || "Steve";
    const uuid2 = crypto.randomUUID().replace(/-/g, "");
    return {
      success: true,
      profile: { name: username, id: uuid2 },
      token: {
        access_token: uuid2,
        client_token: uuid2,
        uuid: uuid2,
...
require$$1$3.ipcMain.handle("game:launch", async (_event, options) => {
  const launcher = new minecraftLauncherCore.Client();
  const opts = {
    authorization: options.auth,
    root: path$m.join(require$$1$3.app.getPath("appData"), ".launcher_1_21_1"),
    version: {
      number: "1.21.1",
      type: "Pikamon",
    ...
    customArgs: [
      ...
      // Try to force window title
      `-DlibraryDirectory=${path$m.join(path$m.join(require$$1$3.app.getPath("appData"), ".launcher_1_21_1"), "libraries")}`,
      "-p",
      [
      ...
      ].map((lib2) => path$m.join(path$m.join(require$$1$3.app.getPath("appData"), ".launcher_1_21_1"), "libraries", lib2)).join(path$m.delimiter),
      "--add-modules",
      "ALL-MODULE-PATH",
      "--add-opens",
...
require$$1$3.app.whenReady().then(() => {
  ...
  main$1.autoUpdater.checkForUpdatesAndNotify().catch((err) => {
